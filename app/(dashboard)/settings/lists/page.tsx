...
const LIST_CONFIGS: ListConfig[] = [
  ...
  },
  {
    ...
    fields: [
      { key: 'name', label: 'Canonical Name', type: 'text', required: true },
      { key: 'aliases', label: 'Aliases', type: 'aliases' },
      { key: 'publicationType', label: 'Publication Type', type: 'text', placeholder: 'Magazine, Illustrated Weekly, Newspaper, etc.' },
      { key: 'country', label: 'Country', type: 'text' },
      { key: 'foundedYear', label: 'Founded Year', type: 'number' },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'publisherId', label: 'Publishing House', type: 'select', relatedList: 'publishers' },
      { key: 'contributors', label: 'Contributors', type: 'textarea', placeholder: 'Illustrated by..., engraved by...' },
      { key: 'country', label: 'Country', type: 'text' },
      { key: 'edition', label: 'Edition', type: 'text', placeholder: 'Second Edition' },
      { key: 'volumeInfo', label: 'Volume Info', type: 'text', placeholder: 'Volume II' },
      { key: 'wikipediaUrl', label: 'Wikipedia URL', type: 'url', placeholder: 'https://en.wikipedia.org/wiki/...' },
      { key: 'bio', label: 'Description', type: 'textarea' },
      { key: 'imageUrl', label: 'Image URL', type: 'url', placeholder: 'https://...' },
...
function ManagedListsContent() {
  ...
  useEffect(() => {
    if (editingItem && editingRowRef.current) {
      setTimeout(() => {
  ...
  useEffect(() => {
    // Clear items and search when switching lists
    setItems([]);
    setSearchQuery('');
    fetchItems();
  }, [activeList]);
                ...
                {loading ? (
                  ...
                  <div className="space-y-2">
                    {filteredItems.map(item => (
                      <div key={item.id} ref={editingItem?.id === item.id ? editingRowRef : null}>
